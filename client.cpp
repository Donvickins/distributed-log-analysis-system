...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "-id" && i + 1 < argc) {
      ...
      try {
        ...
        std::cerr << "[ERROR] Usage: Program -id <client-id> -p <port>"
                  << std::endl;
        return 1;
      }
    }
  ...
  try {
    ...
    stream.socket().set_option(
        tcp::no_delay(true)); // Speed up small chunk sending
    tcp::endpoint server_ = stream.socket().remote_endpoint();
    auto server_ip = server_.address().to_string();
    auto server_port = server_.port();

    std::cout << "[INFO] Connected to server on IP: " << server_ip
    ...
    if (json_data.is_null() || xml_data.empty() || txt_data.empty()) {
      std::cerr << "[ERROR] One or more files are missing or empty."
                << std::endl;
      return 1;
    }
    ...
    std::cout << "[INFO] Processing JSON file: " << get_file_name(json_path)
              << std::endl;
    ...
    std::cout << "[INFO] Processing XML file: " << get_file_name(xml_path)
              << std::endl;
    body_stream << "--" << boundary << "\r\n"
    ...
    std::cout << "[INFO] Processing Text file: " << get_file_name(txt_path)
              << std::endl;
    body_stream << "--" << boundary << "\r\n"
    ...
    std::cout << "[INFO] Total upload size: " << std::fixed
              << std::setprecision(2) << size_mb << " MB\n"
              << std::endl;
    ...
    if (write_ec) {
      std::cerr << "[ERROR] Sending to server: " << write_ec.message()
                << std::endl;
      return 1;
    }
    ...
    std::println("INVALID DATA: {}",
                 static_cast<int>(data.at("invalid_data").as_int64()));
    std::println("TOTAL ENTRIES: {}",
                 static_cast<int>(data.at("total_entries").as_int64()));
