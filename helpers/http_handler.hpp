    ...
    if (content_type.find("multipart/form-data") != std::string::npos) {
      ...
      if (pos != std::string::npos) {
        ...
        return ResponseHandler::bad_request(
            req, "Missing boundary in multipart/form-data");
      }
      ...
      while ((start = body.find(boundary, start)) != std::string::npos) {
        ...
        if (!part_data.empty() && part_data[part_data.size() - 1] == '\n')
          part_data.pop_back();
        if (!part_data.empty() && part_data[part_data.size() - 1] == '\r')
          part_data.pop_back();
        ...
        while (std::getline(hstream, hline)) {
          if (hline.find("Content-Disposition:") != std::string::npos) {
            ...
            if (ctype_pos != std::string::npos) {
              ...
              part_content_type.erase(
                  0, part_content_type.find_first_not_of(" \t\r\n"));
            }
          }
        }
        ...
        if (part_content_type == "application/json") {
          ...
          try {
            auto response =
                save_file(part_data, client_id, client_ip_address, ".json");
          ...
          try {
            auto response =
                save_file(part_data, client_id, client_ip_address, ".xml");
          } catch (const std::exception &e) {
            std::println(stderr, "[ERROR] Saving file: {}", e.what());
            return ResponseHandler::server_error(req, e.what());
          }
          ...
          try {
            auto response =
                save_file(part_data, client_id, client_ip_address, ".txt");
          } catch (const std::exception &e) {
            std::println(stderr, "[ERROR] Saving file: {}", e.what());
            return ResponseHandler::server_error(req, e.what());
          }
