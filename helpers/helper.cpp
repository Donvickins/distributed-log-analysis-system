...
// When given a program path, it returns the program name, else it returns the
// given path
...
std::map<std::string, std::string> save_file(const std::string &content,
  ...
  std::string construct_file_name =
      get_timestamp_str() + "_" + sanitize_ip(ip) + ext;
...
bool is_valid_content_type(const std::string &content_type) {
  ...
  return std::find(valid_types.begin(), valid_types.end(), content_type) !=
         valid_types.end();
}
...
bool is_log_level(const std::string &log_level) {
  const std::vector<std::string> valid_log_level = {"INFO", "DEBUG", "WARN",
                                                    "ERROR", "CRITICAL"};
...
std::string get_file(const fs::path &doc_root, const std::string &file_name,
                     const std::string &clientId, const std::string &file_ext) {
  ...
  std::string file_path =
      path_cat(doc_root.string(),
               "/Client#" + clientId + "/" + file_name + "." + file_ext);
...
void print_response(const boost::json::value &j) {
  ...
  std::println("{:<{}} {:<{}} {:<{}}", "LOG LEVEL", log_level_len, "MESSAGE",
               message_len, "TIMES OCCURRED", times_occurred_len);
  ...
  for (const auto &[log_level, content] : j.as_object()) {
    ...
    for (const auto &[key, value] : content.as_object()) {
      if (is_first) {
        ...
        std::println("{:<{}} {:<{}}", "", log_level_len, key, message_len,
                     value.as_string(), times_occurred_len);
      }
    }

    std::println("{:-<{}}", "",
                 log_level_len + message_len + times_occurred_len);
  }
};