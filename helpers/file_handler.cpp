...
computed_data process_json_request(const std::string &body) {
  ...
  try {
    ...
    for (const auto &log : logs_array) {
      ...
      if (!log.is_object() || log["message"].is_null() ||
          log["log_level"].is_null()) {
        invalid_objects++;
        continue;
      }
  ...
  try {
    ...
    for (const auto &log : logs) {
      const std::string currentLog =
          trim(log.child("log_level").text().as_string());
      const std::string currentLogMsg =
          trim(log.child("message").text().as_string());
